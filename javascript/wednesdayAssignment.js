const events = [
  ...
  const fetchEvents = new Promise((resolve, reject) => {
    ...
    // Immediately check if the events array has data
    if (events.length > 0) {
      resolve(events); // Resolve the promise with the event data
    } else {
      reject('Failed to load event data.'); // Reject if no data
    }
  });
